...
如果当前 6 行没有可观战目标，脚本会向下滑动一屏继续扫描；达到 `runtime.max_swipes_per_tab` 后仍没有目标，才等待 `runtime.interval_seconds`，下一轮会先点当前好友页签把列表归位再重新扫描。
...
- `swipe`：好友列表翻页滑动起止坐标。若滑动后下一组 6 条没有完整对齐，优先调这里。
...
- 找不到目标时不会立刻等待，而是先按配置滑动到下一屏；滑动次数耗尽后再等待并重置列表。
